...
- **Бежево-коричневая гамма заменена на серо-зелёную** по всему сайту: подложки `--surface-alt` `#F2EFE6` → `#EEF2EC`, границы `--border` `#E2DDCE` → `#DFE4DC`, hover-бордер карточек `#B9B29C` → `#A9B3A6` (токены + все инлайн-хардкоды). Тёплая бумага `--bg #FAF8F2` сохранена.
