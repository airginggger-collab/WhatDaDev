...
9. **Показывать вёрстку в чате до пуша (правило пользователя, 2026-08-31):** любое визуальное изменение (новая секция, правка сетки/отступов/цвета, новый компонент) сначала показывается владельцу прямо в диалоге — свёрстанным виджетом, а не описанием словами. Пуш только после явного «да». Правило перекрывает авто-цикл из пункта №3 для визуальных правок; механические фиксы (контраст, битая ссылка, опечатка, JSON-LD) идут без показа.
10. **Чек-листы качества (правило пользователя, 2026-08-31):** перед сдачей экрана и перед публикацией текста проходить `docs/21-quality-checklists.md` — дизайн (шкала кегля, сетка 8pt, радиусы 4/6, все состояния компонента, контраст AA, любое повторяющееся значение в токен), исследование (гипотезы до старта, ИИ-выводы проверять вручную, находки в терминах бизнеса) и текст без ИИ-следа (запрещённая лексика, разная длина абзацев, цифра с источником). Где общая практика расходится с `docs/04-design/tokens.json`, побеждают токены.
...
- Живой сайт: https://what-da-dev.ru/ (технический адрес — https://whatdadev.airg-inggger.workers.dev/)
- Старый сайт заказчика (WordPress, живой): https://whatdadev.ru — **источник правды по контенту** (утверждён заказчиком, v0.64): факты о компании и продуктах сверять оттуда, метрики не выдумывать. Coordo = 3 готовых контура. Рынки — РФ и КЗ.
