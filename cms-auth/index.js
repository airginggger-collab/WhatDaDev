/**
 * WhatDaDev — OAuth-прокси для входа в кабинет /admin/ через GitHub («Sign In with GitHub»).
 *
 * Это отдельный Cloudflare Worker (НЕ основной сайт). Разворачивается один раз вручную.
 * Код взят без изменений из официального проекта sveltia/sveltia-cms-auth (лицензия MIT):
 *   https://github.com/sveltia/sveltia-cms-auth
 *
 * Переменные окружения (задаются при деплое, см. cms-auth/README.md):
 *   ALLOWED_DOMAINS      — домены, где живёт кабинет (whatdadev.airg-inggger.workers.dev,whatdadev.ru)
